...
class DefectFusion:
    ...
    def fit_normal(self, image_paths):
        ...
        if self.anomaly_method != "pca":
            ...
            if self.anomaly_method in {"anoco", "pca_anoco"}:
                self.normal_memory.fit_anoco_calibration(self.anoco_neighbors, self.anoco_query_weight, self.anoco_temperature, self.anoco_anchor_weight)
        if self.dual_branch:
            ...
            if self.anomaly_method != "pca":
                ...
                if self.anomaly_method in {"anoco", "pca_anoco", "pca_knn_anoco"}:
                    self.image_memory.fit_anoco_calibration(self.anoco_neighbors, self.anoco_query_weight, self.anoco_temperature, self.anoco_anchor_weight)
    ...
    def _branch_scores(self, patches, subspace, memory, positions=None, method=None):
        ...
        if method in {"anoco", "pca_anoco"}:
            normal_scores = memory.score_anoco(patches, self.anoco_neighbors, self.anoco_query_weight, self.anoco_temperature, self.anoco_anchor_weight, positions=positions)
            if method == "anoco":
