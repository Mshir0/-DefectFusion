...
class DefectFusion:
    def __init__(self, extractor, *, alpha: float = 0.5, unknown_threshold: float = 0.35, top_k_ratio: float = 0.05, image_score: str = "mtop1p", image_top_ratio: float = 0.01, image_fusion_stage: str = "patch", image_spatial_weight: float = 0.0, type_matching: str = "bidirectional_patch", map_postprocess: str = "none", gaussian_sigma: float = 1.0, anomaly_method: str = "pca", pca_residual_metric: str = "squared_l2", knn_weight: float = 0.5, anoco_neighbors: int = 16, anoco_query_weight: float = 1.0, anoco_temperature: float = 0.07, anoco_weight: float = 0.5, anoco_layer_consensus: bool = False, anoco_layer_top_k: int = 0, memory_max_patches: int = 50000, knn_chunk_size: int = 256, knn_backend: str = "auto", knn_dtype: str = "float32", knn_spatial_radius: float = -1.0, align_training_positions: bool = False, dual_branch: bool = False, fusion_mode: str = "fixed", gate_temperature: float = 1.0, test_augmentations=()):
        ...
        self.anoco_layer_top_k = int(anoco_layer_top_k)
        if self.anoco_layer_top_k < 0 or (self.anoco_layer_top_k > 0 and not self.anoco_layer_consensus):
            raise ValueError("ANoCo layer Top-K requires layer consensus and must be non-negative")
    ...
    def _image_score_bundle(self, patches, positions=None, layer_patches=None):
        ...
        if self.anoco_layer_consensus:
            ...
            stacked_evidence = np.stack(layer_evidence, axis=0)
            if self.anoco_layer_top_k > 0:
                keep = min(self.anoco_layer_top_k, len(stacked_evidence))
                consensus = np.partition(stacked_evidence, -keep, axis=0)[-keep:].mean(axis=0)
            else:
                consensus = np.median(stacked_evidence, axis=0)
            pca_calibrated = subspace.calibrated(pca_scores)
    ...
    def save(self, path):
        state = {
            ...
            "anoco_layer_top_k": self.anoco_layer_top_k,
