...
class DefectFusion:
    def __init__(self, extractor, *, alpha: float = 0.5, unknown_threshold: float = 0.35, top_k_ratio: float = 0.05, image_score: str = "mtop1p", image_top_ratio: float = 0.01, image_fusion_stage: str = "patch", image_spatial_weight: float = 0.0, type_matching: str = "bidirectional_patch", map_postprocess: str = "none", gaussian_sigma: float = 1.0, anomaly_method: str = "pca", pca_residual_metric: str = "squared_l2", knn_weight: float = 0.5, anoco_neighbors: int = 16, anoco_query_weight: float = 1.0, anoco_temperature: float = 0.07, anoco_weight: float = 0.5, anoco_layer_consensus: bool = False, raid_cost_neighbors: int = 1, memory_max_patches: int = 50000, knn_chunk_size: int = 256, knn_backend: str = "auto", knn_dtype: str = "float32", knn_spatial_radius: float = -1.0, align_training_positions: bool = False, dual_branch: bool = False, fusion_mode: str = "fixed", gate_temperature: float = 1.0, test_augmentations=()):
        ...
        self.raid_cost_neighbors = int(raid_cost_neighbors)
        if self.raid_cost_neighbors < 1:
            raise ValueError("raid_cost_neighbors must be positive")
        if self.raid_cost_neighbors > 1 and not self.anoco_layer_consensus:
            raise ValueError("RAID cost aggregation currently requires anoco_layer_consensus")
    ...
    def fit_normal(self, image_paths):
        ...
        if self.dual_branch:
            ...
            if self.anomaly_method != "pca":
                ...
                if self.anoco_layer_consensus:
                    if self.raid_cost_neighbors > 1:
                        self.image_memory.fit_raid_calibration(self.raid_cost_neighbors)
    ...
    def _image_score_bundle(self, patches, positions=None, layer_patches=None):
        ...
        if self.anoco_layer_consensus:
            ...
            if self.raid_cost_neighbors > 1:
                raid_scores = memory.score_raid(patches, self.raid_cost_neighbors, positions=positions)
                consensus = 0.5 * consensus + 0.5 * memory.calibrated_raid(raid_scores)
            pca_calibrated = subspace.calibrated(pca_scores)
    ...
    def save(self, path):
        state = {
            ...
            "raid_cost_neighbors": self.raid_cost_neighbors,
        ...
        if self.dual_branch:
            ...
            state["image_raid_center"] = self.image_memory.raid_center
            state["image_raid_scale"] = self.image_memory.raid_scale
            state["image_raid_calibration_scores"] = self.image_memory.raid_calibration_scores.tolist() if self.image_memory.raid_calibration_scores is not None else None
            if self.anoco_layer_consensus:
    ...
    def load(cls, path, extractor):
        ...
        if memory_file:
            ...
            if obj.dual_branch and "image_features" in memory:
                ...
                obj.image_memory.raid_center = float(state.get("image_raid_center", 0.0))
                obj.image_memory.raid_scale = float(state.get("image_raid_scale", 1.0))
                image_raid_calibration = state.get("image_raid_calibration_scores")
                obj.image_memory.raid_calibration_scores = None if image_raid_calibration is None else np.asarray(image_raid_calibration, dtype=np.float64)
