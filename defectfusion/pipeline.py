...
class DefectFusion:
    def __init__(self, extractor, *, alpha: float = 0.5, unknown_threshold: float = 0.35, top_k_ratio: float = 0.05, image_score: str = "mtop1p", image_top_ratio: float = 0.01, image_fusion_stage: str = "patch", image_spatial_weight: float = 0.0, type_matching: str = "bidirectional_patch", map_postprocess: str = "none", gaussian_sigma: float = 1.0, anomaly_method: str = "pca", pca_residual_metric: str = "squared_l2", knn_weight: float = 0.5, anoco_neighbors: int = 16, anoco_query_weight: float = 1.0, anoco_temperature: float = 0.07, anoco_weight: float = 0.5, anoco_layer_consensus: bool = False, normal_view_top_k: int = 0, memory_max_patches: int = 50000, knn_chunk_size: int = 256, knn_backend: str = "auto", knn_dtype: str = "float32", knn_spatial_radius: float = -1.0, align_training_positions: bool = False, dual_branch: bool = False, fusion_mode: str = "fixed", gate_temperature: float = 1.0, test_augmentations=()):
        ...
        self.normal_view_top_k = int(normal_view_top_k)
        if self.normal_view_top_k < 0 or (self.normal_view_top_k > 0 and not self.anoco_layer_consensus):
            raise ValueError("Normal view selection requires layer consensus and a non-negative Top-K")
                ...
                if self.anoco_layer_consensus:
                    ...
                    for layer_index in range(layer_count):
                        ...
                        view_ids = np.concatenate([
                            np.full(batch[layer_index].shape[0], index, dtype=np.int32)
                        ...
                        layer_memory.fit(layer_features, memory_positions, view_ids=view_ids)
                        layer_memory.fit_anoco_calibration(
                            self.anoco_neighbors, self.anoco_query_weight, self.anoco_temperature,
                        )
                        if self.normal_view_top_k > 0:
                            layer_memory.fit_view_index(self.normal_view_top_k)
    ...
    def save(self, path):
        state = {
            ...
            "normal_view_top_k": self.normal_view_top_k,
        ...
        if self.normal_memory.features is not None:
            ...
            if self.dual_branch and self.image_memory.features is not None:
                ...
                for index, memory in enumerate(self.image_layer_memories):
                    ...
                    if memory.view_descriptors is not None:
                        memory_data[f"image_layer_{index}_view_ids"] = memory.view_ids
                        memory_data[f"image_layer_{index}_view_descriptors"] = memory.view_descriptors.astype(np.float16)
                        memory_data[f"image_layer_{index}_view_descriptor_ids"] = memory.view_descriptor_ids
    ...
    def load(cls, path, extractor):
        ...
        obj.normal_view_top_k = int(state.get("normal_view_top_k", 0))
        ...
        if memory_file:
            ...
            if obj.dual_branch and "image_features" in memory:
                ...
                for index, layer_state in enumerate(state.get("image_layer_anoco", [])):
                    ...
                    view_key = f"image_layer_{index}_view_ids"
                    if view_key in memory:
                        layer_memory.view_ids = memory[view_key].astype(np.int32)
                        layer_memory.view_descriptors = memory[f"image_layer_{index}_view_descriptors"].astype(np.float32)
                        layer_memory.view_descriptor_ids = memory[f"image_layer_{index}_view_descriptor_ids"].astype(np.int32)
                        layer_memory.view_top_k = obj.normal_view_top_k
