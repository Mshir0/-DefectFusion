...
class DefectFusion:
    def __init__(self, extractor, *, alpha: float = 0.5, unknown_threshold: float = 0.35, top_k_ratio: float = 0.05, image_score: str = "mtop1p", image_top_ratio: float = 0.01, image_fusion_stage: str = "patch", image_spatial_weight: float = 0.0, image_min_component_size: int = 1, type_matching: str = "bidirectional_patch", map_postprocess: str = "none", gaussian_sigma: float = 1.0, anomaly_method: str = "pca", pca_residual_metric: str = "squared_l2", knn_weight: float = 0.5, anoco_neighbors: int = 16, anoco_query_weight: float = 1.0, anoco_temperature: float = 0.07, anoco_affinity: str = "softmax", anoco_anchor_ranking: str = "mean", anoco_norm_compatibility: bool = False, anoco_weight: float = 0.5, pixel_anoco_weight: float = 0.0, pixel_anoco_norm_compatibility: bool = False, anoco_layer_consensus: bool = False, memory_max_patches: int = 50000, normal_fit_max_patches: int = 0, knn_chunk_size: int = 256, knn_backend: str = "auto", knn_dtype: str = "float32", knn_spatial_radius: float = -1.0, image_knn_spatial_radius: float | None = None, align_training_positions: bool = False, align_image_training_positions: bool | None = None, dual_branch: bool = False, fusion_mode: str = "fixed", gate_temperature: float = 1.0, test_augmentations=(), pixel_image_size: int | None = None, image_head_image_size: int | None = None, secondary_pixel_image_size: int | None = None, pixel_multiscale_weight: float = 0.5):
        ...
        image_spatial_radius = knn_spatial_radius if image_knn_spatial_radius is None else image_knn_spatial_radius
        self.image_memory = NormalPatchMemory(memory_max_patches, knn_chunk_size, backend=knn_backend, device=knn_device if self.dual_branch else None, dtype=knn_dtype, spatial_radius=image_spatial_radius) if self.dual_branch else None
        self.alpha = alpha
        ...
        if normal_fit_max_patches < 0:
            raise ValueError("normal_fit_max_patches must be non-negative")
        self.normal_fit_max_patches = int(normal_fit_max_patches)
        ...
        self.align_image_training_positions = self.align_training_positions if align_image_training_positions is None else bool(align_image_training_positions)
        ...
        if not 0 <= pixel_anoco_weight <= 1:
            raise ValueError("pixel_anoco_weight must be in [0, 1]")
        if pixel_anoco_weight > 0 and anomaly_method not in {"pca_knn", "pca_knn_anoco"}:
            raise ValueError("pixel_anoco_weight requires pca_knn or pca_knn_anoco")
        if pixel_anoco_weight > 0 and fusion_mode != "fixed":
            raise ValueError("pixel_anoco_weight currently requires fixed fusion")
        ...
        self.pixel_anoco_weight = float(pixel_anoco_weight)
        self.pixel_anoco_norm_compatibility = bool(pixel_anoco_norm_compatibility)
    ...
    def fit_normal(self, image_paths):
        ...
        per_image_limit = per_image_remainder = 0
        if self.normal_fit_max_patches > 0 and image_paths:
            if self.normal_fit_max_patches < len(image_paths):
                raise ValueError("normal_fit_max_patches must cover at least one patch per normal image")
            per_image_limit, per_image_remainder = divmod(self.normal_fit_max_patches, len(image_paths))

        def sample_indices(count, image_index):
            if self.normal_fit_max_patches <= 0:
                return np.arange(count)
            image_limit = per_image_limit + int(image_index < per_image_remainder)
            if count <= image_limit:
                return np.arange(count)
            if image_limit == 0:
                return np.empty(0, dtype=np.int64)
            return np.linspace(0, count - 1, image_limit, dtype=np.int64)
        ...
        for image_index, path in enumerate(image_paths):
            ...
            patch_fit_indices = sample_indices(len(patches), image_index)
            if self.dual_branch:
                image_fit_indices = sample_indices(len(image_patches), image_index)
                image_patch_batches.append(image_patches[image_fit_indices])
                if self.anoco_layer_consensus:
                    image_layer_batches.append(image_layers[:, image_fit_indices])
            patch_batches.append(patches[patch_fit_indices])
            positions, valid = self._training_positions(grid, inverse_position_matrix)
            if self.align_training_positions:
                memory_patches = patches[valid]
                memory_fit_indices = sample_indices(len(memory_patches), image_index)
                memory_patch_batches.append(memory_patches[memory_fit_indices])
                memory_position_batches.append(positions[memory_fit_indices])
            else:
                position_batches.append(positions[patch_fit_indices])
            if self.dual_branch:
                image_positions, image_valid = self._training_positions(
                    image_grid, inverse_position_matrix,
                    align=self.align_image_training_positions,
                )
                if self.align_image_training_positions:
                    image_memory_patches = image_patches[image_valid]
                    image_memory_layers = image_layers[:, image_valid] if self.anoco_layer_consensus else None
                    image_memory_fit_indices = sample_indices(len(image_memory_patches), image_index)
                    image_memory_patch_batches.append(image_memory_patches[image_memory_fit_indices])
                    image_memory_position_batches.append(image_positions[image_memory_fit_indices])
                    if self.anoco_layer_consensus:
                        image_layer_memory_batches.append(image_memory_layers[:, image_memory_fit_indices])
                else:
                    image_position_batches.append(image_positions[image_fit_indices])
            if self.secondary_pixel_image_size is not None:
                ...
                secondary_fit_indices = sample_indices(len(secondary_patches), image_index)
                secondary_patch_batches.append(secondary_patches[secondary_fit_indices])
                if self.align_training_positions:
                    secondary_memory_patches = secondary_patches[secondary_valid]
                    secondary_memory_fit_indices = sample_indices(len(secondary_memory_patches), image_index)
                    secondary_memory_patch_batches.append(secondary_memory_patches[secondary_memory_fit_indices])
                    secondary_position_batches.append(secondary_positions[secondary_memory_fit_indices])
                else:
                    secondary_position_batches.append(secondary_positions[secondary_fit_indices])
        ...
        if self.anomaly_method != "pca":
            ...
            if self.anomaly_method in {"anoco", "pca_anoco"} or self.pixel_anoco_weight > 0:
                norm_compatibility = self.pixel_anoco_norm_compatibility if self.pixel_anoco_weight > 0 else self.anoco_norm_compatibility
                self.normal_memory.fit_anoco_calibration(self.anoco_neighbors, self.anoco_query_weight, self.anoco_temperature, affinity=self.anoco_affinity, anchor_ranking=self.anoco_anchor_ranking, norm_compatibility=norm_compatibility)
        if self.secondary_pixel_image_size is not None:
            ...
            if self.anomaly_method != "pca":
                ...
                if self.anomaly_method in {"anoco", "pca_anoco"} or self.pixel_anoco_weight > 0:
                    norm_compatibility = self.pixel_anoco_norm_compatibility if self.pixel_anoco_weight > 0 else self.anoco_norm_compatibility
                    self.secondary_normal_memory.fit_anoco_calibration(self.anoco_neighbors, self.anoco_query_weight, self.anoco_temperature, affinity=self.anoco_affinity, anchor_ranking=self.anoco_anchor_ranking, norm_compatibility=norm_compatibility)
        if self.dual_branch:
            image_features = np.concatenate(image_patch_batches, axis=0)
            image_memory_positions = np.concatenate(image_memory_position_batches if self.align_image_training_positions else image_position_batches, axis=0)
            self.image_subspace.fit(image_features)
            if self.anomaly_method != "pca":
                image_memory_features = np.concatenate(image_memory_patch_batches, axis=0) if self.align_image_training_positions else image_features
                ...
                if self.anoco_layer_consensus:
                    ...
                    for layer_index in range(layer_count):
                        source = image_layer_memory_batches if self.align_image_training_positions else image_layer_batches
                        ...
                        layer_memory = NormalPatchMemory(
                            ...
                            backend=self.image_memory.backend, device=self.image_memory.device,
                            dtype=self.image_memory.dtype, spatial_radius=self.image_memory.spatial_radius,
                        )
    ...
    def _training_positions(self, grid, inverse_position_matrix, align=None):
        positions = self._patch_positions(grid)
        align = self.align_training_positions if align is None else bool(align)
        if not align or inverse_position_matrix is None:
    ...
    def _anomaly_scores(self, patches, positions=None):
        ...
        bundle = self._branch_scores(patches, self.subspace, self.normal_memory, positions, method)
        return self._pixel_anoco_fusion(bundle, patches, self.subspace, self.normal_memory, positions)

    def _secondary_anomaly_scores(self, patches, positions=None):
        method = "pca_knn" if self.anomaly_method == "pca_knn_anoco" else self.anomaly_method
        bundle = self._branch_scores(patches, self.secondary_subspace, self.secondary_normal_memory, positions, method)
        return self._pixel_anoco_fusion(bundle, patches, self.secondary_subspace, self.secondary_normal_memory, positions)

    def _pixel_anoco_fusion(self, bundle, patches, subspace, memory, positions):
        if self.pixel_anoco_weight <= 0:
            return bundle
        _, pca_scores, knn_scores, gate = bundle
        anoco_scores = memory.score_anoco(
            patches, self.anoco_neighbors, self.anoco_query_weight,
            self.anoco_temperature, positions=positions, affinity=self.anoco_affinity,
            anchor_ranking=self.anoco_anchor_ranking,
            norm_compatibility=self.pixel_anoco_norm_compatibility,
        )
        base = ((1.0 - self.knn_weight) * subspace.calibrated(pca_scores)
                + self.knn_weight * memory.calibrated(knn_scores))
        fused = ((1.0 - self.pixel_anoco_weight) * base
                 + self.pixel_anoco_weight * memory.calibrated_anoco(anoco_scores))
        return fused, pca_scores, knn_scores, gate
    ...
    def save(self, path):
        state = {
            ...
            "pixel_anoco_weight": self.pixel_anoco_weight,
            "pixel_anoco_norm_compatibility": self.pixel_anoco_norm_compatibility,
            "anoco_layer_consensus": self.anoco_layer_consensus,
            "memory_max_patches": self.normal_memory.max_patches,
            "normal_fit_max_patches": self.normal_fit_max_patches,
            ...
            "image_knn_spatial_radius": self.image_memory.spatial_radius if self.image_memory is not None else None,
            "align_training_positions": self.align_training_positions,
            "align_image_training_positions": self.align_image_training_positions,
