...
VisA 各个 normal/defect shot 结果中的低表现类别可通过同一脚本单独调优，不会覆盖正式 shot 结果：

```bash
MODE=tune bash scripts/evaluate_visa_shots.sh
```

`TUNING_FAMILY=detection` 会从 1/2/4/8 normal-shot 的正式结果中自动选出 `balanced_accuracy` 低于 `DETECTION_BALANCED_THRESHOLD` 的类别，测试增强阈值校准；`TUNING_FAMILY=typing` 会从 8 normal-shot + 1/2/4/8 defect-shot 结果中自动选出 `defect_type_macro_f1` 低于 `TYPE_MACRO_F1_THRESHOLD` 的类别，测试 patch 匹配方案。输出位于 `outputs/visa-tuning/`，已完成方案默认跳过。

运行完成后汇总调优结果：

```bash
python -m defectfusion.aggregate \
  --input outputs/visa-tuning \
  --output outputs/visa-tuning-summary
```
