...
- [x] P5. Test norm compatibility only after P4. Multiply the winning affinity by `min(norm_q, norm_r) / max(norm_q, norm_r)`, then renormalize each query's edge weights to sum to one. The setting was neutral: every image and pixel metric matched the P3 mean-ranking baseline exactly. Keep it disabled by default. Command and per-category CSV are recorded in `experiments/visa-anoco-norm-compatibility.md` and `.csv`.
