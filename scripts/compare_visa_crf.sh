...
# One-category VisA PCA anomaly-map ablation. Both runs use identical data,
# model, normal fitting, and source-disjoint threshold calibration; only map
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/visa-crf-ablation-loo}"
SKIP_COMPLETED="${SKIP_COMPLETED:-0}"

if [[ ! -d "$VISA_DATA_ROOT" ]]; then
  printf 'VISA_DATA_ROOT does not exist: %s\n' "$VISA_DATA_ROOT" >&2
  exit 2
fi
...
if [[ -n "$VISA_SPLIT_CSV" ]]; then
  dataset_args+=(--split-csv "$VISA_SPLIT_CSV")
fi

# Preserve the category-specific preprocessing used by the main VisA PCA run.
case "$CATEGORY" in
  macaroni2|pcb2|pcb3)
    dataset_args+=(--image-size-override "$CATEGORY=896")
    ;;
esac
case "$CATEGORY" in
  macaroni1|macaroni2)
    dataset_args+=(--affine-categories "$CATEGORY")
    ;;
esac
...
run_mode() {
  ...
  if [[ "$SKIP_COMPLETED" == "1" && -f "$output/summary.csv" ]]; then
    printf '[visa-crf-ablation] skipping %s (complete: %s)\n' "$mode" "$output/summary.csv"
    return
  fi
  printf '[visa-crf-ablation] category=%s mode=%s\n' "$CATEGORY" "$mode"
...
print(f"[visa-crf-ablation] category={category}")
...
print(f"[visa-crf-ablation] comparison: {output_path}")
PY
