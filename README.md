...
preserves the validated query-only weighting. `--anoco-drift-mode
calibrated_sum` independently normal-calibrates squared feature displacement
and angular displacement before combining them with `--anoco-drift-weight`;
the default `product` preserves the validated multiplicative energy. The implementation reuses the normal memory,
